...
static const char WINDOW_TITLE[] = "TD02 Exo 05";
...
typedef struct point_link {
    int x, y; // Position 2D du vertex
    unsigned char r, g, b; // Couleur du vertex
    struct point_link *next; // Suivant
} Point, *PointList;

typedef struct primitive_link {
    GLenum primitiveType; // Type de primitive
    PointList vertices; // Liste des vertex
...
Point* allocVertex(int x, int y, unsigned char r, unsigned char g, unsigned char b){
...
void deleteVertices(PointList* list){
    free(list);
}

void addVertex(Point* vertex, PointList* list){
    Point* currentVertex = list;
...
void drawVertices(PointList list){
    Point* currentVertex = list;
    while(currentVertex->next!=NULL){
        glColor3ub(currentVertex->r, currentVertex->g, currentVertex->b);
        glVertex2f(currentVertex->x, currentVertex->y);
    }
}

Primitive* allocPrimitive(GLenum primitiveType){
    Primitive* result = (Primitive*) malloc(10*sizeof(Primitive));
    return result;
}
